<#
Builds OptiScaler.dll locally the same way the "Build OptiScaler 0.10"
GitHub Actions workflow does: clone optiscaler/OptiScaler, apply this repo's
release/0.10 patch stack, and build OptiScaler.vcxproj with MSBuild.

Usage:
  .\scripts\build-optiscaler-0.10-dll.ps1
  .\scripts\build-optiscaler-0.10-dll.ps1 -OptiScalerRef v1.2.3
  .\scripts\build-optiscaler-0.10-dll.ps1 -NoPause
#>

param(
    [string] $OptiScalerRef = "master",
    [switch] $NoPause
)
...
$PatchSourceBranch = "release/0.10"
$OutputDir        = "C:\GoogleDrive\00. OptiScaler\06. OptiSensor\release-0.10"
$CacheDirName     = ".work/OptiScaler-0.10"
